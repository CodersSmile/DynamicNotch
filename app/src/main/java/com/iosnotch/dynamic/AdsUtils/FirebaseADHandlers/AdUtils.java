...
import com.iosnotch.dynamic.AdsUtils.Utils.ConnectionDetector;
import com.iosnotch.dynamic.AdsUtils.Utils.Constants;
import com.iosnotch.dynamic.AdsUtils.Utils.Global;
...
public class AdUtils {
        ...
        if (cd.isConnectingToInternet()) {
            ...
            RewardedAd.load(activity, Constants.adsJsonPOJO.getParameters().getRewarded_ad().getValueType(), adRequest, new RewardedAdLoadCallback() {
                ...
                public void onAdFailedToLoad(@NonNull LoadAdError loadAdError) {
                    Global.printLog("RewardedAdLoadCallback: ", loadAdError.toString());
                }
                ...
                public void onAdLoaded(@NonNull RewardedAd rewardedAd) {
                    ...
                    rewardedAd.setFullScreenContentCallback(new FullScreenContentCallback() {
                        ...
                        public void onAdFailedToShowFullScreenContent(AdError adError) {
                            Global.printLog("onAdFailedToShowFullScreenContent: ", adError.toString());
                        }

                        @Override
                        public void onAdDismissedFullScreenContent() {
                            Global.printLog("onAdFailedToShowFullScreenContent: ", "onAdDismissedFullScreenContent");
                        }
                    });
                }
    ...
    public static void showNativeAd(final Activity activity, String nativeAd, final LinearLayout adContainer, final boolean isFullScreenAd) {
        if (Boolean.parseBoolean(Constants.adsJsonPOJO.getParameters().getShowAd().getDefaultValue().getValue())) {
            cd = new ConnectionDetector(activity);
            if (!Global.isEmptyStr(nativeAd)) {
                if (cd.isConnectingToInternet()) {
                    try {
    ...
    public static void showBannerAd(final LinearLayout adContainer, Activity activity) {
        cd = new ConnectionDetector(activity);


        if (!Global.isEmptyStr(Constants.adsJsonPOJO.getParameters().getBanner_id().getDefaultValue().getValue())) {
            if (cd.isConnectingToInternet()) {
...
//    }
    ...
    public static void showInterstitialAd(final Activity activity, final AppInterfaces.InterStitialADInterface interStitialADInterface) {
        if (Boolean.parseBoolean(Constants.adsJsonPOJO.getParameters().getShowAd().getDefaultValue().getValue())) {
    ...
    public static void showAppOpenAd(final Activity activity, final AppInterfaces.AppOpenADInterface appOpenADInterface) {
        if (Boolean.parseBoolean(Constants.adsJsonPOJO.getParameters().getShowAd().getDefaultValue().getValue())) {

            global = new Global(activity);

            AdRequest adRequest = new AdRequest.Builder().build();
            if (!Global.isEmptyStr(Constants.adsJsonPOJO.getParameters().getApp_open_ad().getDefaultValue().getValue())) {
                global.showProgressDialog(activity, Global.PLEASE_WAIT);
    ...
    private static void showAdIfAvailable(Activity activity, final AppInterfaces.AppOpenADInterface appOpenADInterface) {
...
//                            fetchAd();
    ...
    private static void loadInterstitialAd(Activity activity, final AppInterfaces.InterStitialADInterface interStitialADInterface) {
